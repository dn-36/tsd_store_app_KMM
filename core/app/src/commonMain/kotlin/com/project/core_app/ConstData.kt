package com.project.core_app

import com.project.core_app.components.menu_bottom_bar.model.CountryData


object ConstData {
    const val TOKEN = "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9..1VyqXO-OQbnXx--dH1kGNp9Jad8H089w2U3nxG72C5ftOVvRd3dYhnaiaRFVSV2oDsLBXVAgdQ7NikGCkT6crMd7kZ4HUO6feHJwgBzmUZ0z3t2yjGSTCuhkF8zdT32WlGSLe2vdC3RZvkajKnsj0XeKBGLfUVW6cv8QF1FH4DYBTWIMPftGlWQHiJzl6qmgB--3tbJvdALxn4jd-i9DBcbF8Qwf1Gj9eJJxsqw-2CIw3i_lgx-2IgPKiZ1LCTRWum_Nn5QV8R-2K1CQGAOXzsI1VcQqSUWKviwNG1Z9Wn5dUpeahh-3R8KpD1zbBoSMXYM-g-4GmiSJmh5rZbtFygTtTrdi1zcNGSBs4jMmt9fwDSvGOYSJ3BFK6fn-PO9WUsPX-YM2luhkG-fgpJQDK8yPm3iNSkq7nyfiGw6tiVC8H94CSKrW_RkLzjIw_8BNuaiByHNPD3M6E8f7rOX24xq4D56lEMlqVNzYpfTjtd8vO-e0zseB_xC_xxhDLivfzvmUKEVHdTurKKD_YfYPTP6LGGlh0kxmAluPIYMApu7_FMIDHWwXi64wKQD1vLS9I-758rMIUaw7xDM36t4LYHvTXOjnfWfCoK4Q4oExB_jSDUcKzt9ZpNJhSt-la6n75ADkiDh7zgRBlNpT1-kMViA-PKdlh-vEaXTJBaGjfNc"
   val  LIB_COUNTRIES: List<CountryData> = mutableListOf(
        CountryData("ad", "+376", "Andorra"),
        CountryData("ae", "+971", "United Arab Emirates (UAE)"),
        CountryData("af", "+93", "Afghanistan"),
        CountryData("ag", "+1", "Antigua and Barbuda"),
        CountryData("ai", "+1", "Anguilla"),
        CountryData("al", "+355", "Albania"),
        CountryData("am", "+374", "Armenia"),
        CountryData("ao", "+244", "Angola"),
        CountryData("aq", "+672", "Antarctica"),
        CountryData("ar", "+54", "Argentina"),
        CountryData("as", "+1", "American Samoa"),
        CountryData("at", "+43", "Austria"),
        CountryData("au", "+61", "Australia"),
        CountryData("aw", "+297", "Aruba"),
        CountryData("ax", "+358", "Åland Islands"),
        CountryData("az", "+994", "Azerbaijan"),
        CountryData("ba", "+387", "Bosnia And Herzegovina"),
        CountryData("bb", "+1", "Barbados"),
        CountryData("bd", "+880", "Bangladesh"),
        CountryData("be", "+32", "Belgium"),
        CountryData("bf", "+226", "Burkina Faso"),
        CountryData("bg", "+359", "Bulgaria"),
        CountryData("bh", "+973", "Bahrain"),
        CountryData("bi", "+257", "Burundi"),
        CountryData("bj", "+229", "Benin"),
        CountryData("bl", "+590", "Saint Barthélemy"),
        CountryData("bm", "+1", "Bermuda"),
        CountryData("bn", "+673", "Brunei Darussalam"),
        CountryData("bo", "+591", "Bolivia, Plurinational State Of"),
        CountryData("br", "+55", "Brazil"),
        CountryData("bs", "+1", "Bahamas"),
        CountryData("bt", "+975", "Bhutan"),
        CountryData("bw", "+267", "Botswana"),
        CountryData("by", "+375", "Belarus"),
        CountryData("bz", "+501", "Belize"),
        CountryData("ca", "+1", "Canada"),
        CountryData("cc", "+61", "Cocos (keeling) Islands"),
        CountryData("cd", "+243", "Congo, The Democratic Republic Of The"),
        CountryData("cf", "+236", "Central African Republic"),
        CountryData("cg", "+242", "Congo"),
        CountryData("ch", "+41", "Switzerland"),
        CountryData("ci", "+225", "Côte D'ivoire"),
        CountryData("ck", "+682", "Cook Islands"),
        CountryData("cl", "+56", "Chile"),
        CountryData("cm", "+237", "Cameroon"),
        CountryData("cn", "+86", "China"),
        CountryData("co", "+57", "Colombia"),
        CountryData("cr", "+506", "Costa Rica"),
        CountryData("cu", "+53", "Cuba"),
        CountryData("cv", "+238", "Cape Verde"),
        CountryData("cw", "+599", "Curaçao"),
        CountryData("cx", "+61", "Christmas Island"),
        CountryData("cy", "+357", "Cyprus"),
        CountryData("cz", "+420", "Czech Republic"),
        CountryData("de", "+49", "Germany"),
        CountryData("dj", "+253", "Djibouti"),
        CountryData("dk", "+45", "Denmark"),
        CountryData("dm", "+1", "Dominica"),
        CountryData("do", "+1", "Dominican Republic"),
        CountryData("dz", "+213", "Algeria"),
        CountryData("ec", "+593", "Ecuador"),
        CountryData("ee", "+372", "Estonia"),
        CountryData("eg", "+20", "Egypt"),
        CountryData("er", "+291", "Eritrea"),
        CountryData("es", "+34", "Spain"),
        CountryData("et", "+251", "Ethiopia"),
        CountryData("fi", "+358", "Finland"),
        CountryData("fj", "+679", "Fiji"),
        CountryData("fk", "+500", "Falkland Islands (malvinas)"),
        CountryData("fm", "+691", "Micronesia, Federated States Of"),
        CountryData("fo", "+298", "Faroe Islands"),
        CountryData("fr", "+33", "France"),
        CountryData("ga", "+241", "Gabon"),
        CountryData("gb", "+44", "United Kingdom"),
        CountryData("gd", "+1", "Grenada"),
        CountryData("ge", "+995", "Georgia"),
        CountryData("gf", "+594", "French Guyana"),
        CountryData("gh", "+233", "Ghana"),
        CountryData("gi", "+350", "Gibraltar"),
        CountryData("gl", "+299", "Greenland"),
        CountryData("gm", "+220", "Gambia"),
        CountryData("gn", "+224", "Guinea"),
        CountryData("gp", "+450", "Guadeloupe"),
        CountryData("gq", "+240", "Equatorial Guinea"),
        CountryData("gr", "+30", "Greece"),
        CountryData("gt", "+502", "Guatemala"),
        CountryData("gu", "+1", "Guam"),
        CountryData("gw", "+245", "Guinea-bissau"),
        CountryData("gy", "+592", "Guyana"),
        CountryData("hk", "+852", "Hong Kong"),
        CountryData("hn", "+504", "Honduras"),
        CountryData("hr", "+385", "Croatia"),
        CountryData("ht", "+509", "Haiti"),
        CountryData("hu", "+36", "Hungary"),
        CountryData("id", "+62", "Indonesia"),
        CountryData("ie", "+353", "Ireland"),
        CountryData("il", "+972", "Israel"),
        CountryData("im", "+44", "Isle Of Man"),
        CountryData("is", "+354", "Iceland"),
        CountryData("in", "+91", "India"),
        CountryData("io", "+246", "British Indian Ocean Territory"),
        CountryData("iq", "+964", "Iraq"),
        CountryData("ir", "+98", "Iran, Islamic Republic Of"),
        CountryData("it", "+39", "Italy"),
        CountryData("je", "+44", "Jersey "),
        CountryData("jm", "+1", "Jamaica"),
        CountryData("jo", "+962", "Jordan"),
        CountryData("jp", "+81", "Japan"),
        CountryData("ke", "+254", "Kenya"),
        CountryData("kg", "+996", "Kyrgyzstan"),
        CountryData("kh", "+855", "Cambodia"),
        CountryData("ki", "+686", "Kiribati"),
        CountryData("km", "+269", "Comoros"),
        CountryData("kn", "+1", "Saint Kitts and Nevis"),
        CountryData("kp", "+850", "North Korea"),
        CountryData("kr", "+82", "South Korea"),
        CountryData("kw", "+965", "Kuwait"),
        CountryData("ky", "+1", "Cayman Islands"),
        CountryData("kz", "+7", "Kazakhstan"),
        CountryData("la", "+856", "Lao People's Democratic Republic"),
        CountryData("lb", "+961", "Lebanon"),
        CountryData("lc", "+1", "Saint Lucia"),
        CountryData("li", "+423", "Liechtenstein"),
        CountryData("lk", "+94", "Sri Lanka"),
        CountryData("lr", "+231", "Liberia"),
        CountryData("ls", "+266", "Lesotho"),
        CountryData("lt", "+370", "Lithuania"),
        CountryData("lu", "+352", "Luxembourg"),
        CountryData("lv", "+371", "Latvia"),
        CountryData("ly", "+218", "Libya"),
        CountryData("ma", "+212", "Morocco"),
        CountryData("mc", "+377", "Monaco"),
        CountryData("md", "+373", "Moldova, Republic Of"),
        CountryData("me", "+382", "Montenegro"),
        CountryData("mf", "+590", "Saint Martin"),
        CountryData("mg", "+261", "Madagascar"),
        CountryData("mh", "+692", "Marshall Islands"),
        CountryData("mk", "+389", "Macedonia (FYROM)"),
        CountryData("ml", "+223", "Mali"),
        CountryData("mm", "+95", "Myanmar"),
        CountryData("mn", "+976", "Mongolia"),
        CountryData("mo", "+853", "Macau"),
        CountryData("mp", "+1", "Northern Mariana Islands"),
        CountryData("mq", "+596", "Martinique"),
        CountryData("mr", "+222", "Mauritania"),
        CountryData("ms", "+1", "Montserrat"),
        CountryData("mt", "+356", "Malta"),
        CountryData("mu", "+230", "Mauritius"),
        CountryData("mv", "+960", "Maldives"),
        CountryData("mw", "+265", "Malawi"),
        CountryData("mx", "+52", "Mexico"),
        CountryData("my", "+60", "Malaysia"),
        CountryData("mz", "+258", "Mozambique"),
        CountryData("na", "+264", "Namibia"),
        CountryData("nc", "+687", "New Caledonia"),
        CountryData("ne", "+227", "Niger"),
        CountryData("nf", "+672", "Norfolk Islands"),
        CountryData("ng", "+234", "Nigeria"),
        CountryData("ni", "+505", "Nicaragua"),
        CountryData("nl", "+31", "Netherlands"),
        CountryData("no", "+47", "Norway"),
        CountryData("np", "+977", "Nepal"),
        CountryData("nr", "+674", "Nauru"),
        CountryData("nu", "+683", "Niue"),
        CountryData("nz", "+64", "New Zealand"),
        CountryData("om", "+968", "Oman"),
        CountryData("pa", "+507", "Panama"),
        CountryData("pe", "+51", "Peru"),
        CountryData("pf", "+689", "French Polynesia"),
        CountryData("pg", "+675", "Papua New Guinea"),
        CountryData("ph", "+63", "Philippines"),
        CountryData("pk", "+92", "Pakistan"),
        CountryData("pl", "+48", "Poland"),
        CountryData("pm", "+508", "Saint Pierre And Miquelon"),
        CountryData("pn", "+870", "Pitcairn Islands"),
        CountryData("pr", "+1", "Puerto Rico"),
        CountryData("ps", "+970", "Palestine"),
        CountryData("pt", "+351", "Portugal"),
        CountryData("pw", "+680", "Palau"),
        CountryData("py", "+595", "Paraguay"),
        CountryData("qa", "+974", "Qatar"),
        CountryData("re", "+262", "Réunion"),
        CountryData("ro", "+40", "Romania"),
        CountryData("rs", "+381", "Serbia"),
        CountryData("com", "+7", "Russian Federation"),
        CountryData("rw", "+250", "Rwanda"),
        CountryData("sa", "+966", "Saudi Arabia"),
        CountryData("sb", "+677", "Solomon Islands"),
        CountryData("sc", "+248", "Seychelles"),
        CountryData("sd", "+249", "Sudan"),
        CountryData("se", "+46", "Sweden"),
        CountryData("sg", "+65", "Singapore"),
        CountryData("sh", "+290", "Saint Helena, Ascension And Tristan Da Cunha"),
        CountryData("si", "+386", "Slovenia"),
        CountryData("sk", "+421", "Slovakia"),
        CountryData("sl", "+232", "Sierra Leone"),
        CountryData("sm", "+378", "San Marino"),
        CountryData("sn", "+221", "Senegal"),
        CountryData("so", "+252", "Somalia"),
        CountryData("sr", "+597", "Suriname"),
        CountryData("ss", "+211", "South Sudan"),
        CountryData("st", "+239", "Sao Tome And Principe"),
        CountryData("sv", "+503", "El Salvador"),
        CountryData("sx", "+1", "Sint Maarten"),
        CountryData("sy", "+963", "Syrian Arab Republic"),
        CountryData("sz", "+268", "Swaziland"),
        CountryData("tc", "+1", "Turks and Caicos Islands"),
        CountryData("td", "+235", "Chad"),
        CountryData("tg", "+228", "Togo"),
        CountryData("th", "+66", "Thailand"),
        CountryData("tj", "+992", "Tajikistan"),
        CountryData("tk", "+690", "Tokelau"),
        CountryData("tl", "+670", "Timor-leste"),
        CountryData("tm", "+993", "Turkmenistan"),
        CountryData("tn", "+216", "Tunisia"),
        CountryData("to", "+676", "Tonga"),
        CountryData("tr", "+90", "Turkey"),
        CountryData("tt", "+1", "Trinidad &amp; Tobago"),
        CountryData("tv", "+688", "Tuvalu"),
        CountryData("tw", "+886", "Taiwan"),
        CountryData("tz", "+255", "Tanzania, United Republic Of"),
        CountryData("ua", "+380", "Ukraine"),
        CountryData("ug", "+256", "Uganda"),
        CountryData("us", "+1", "United States"),
        CountryData("uy", "+598", "Uruguay"),
        CountryData("uz", "+998", "Uzbekistan"),
        CountryData("va", "+379", "Holy See (vatican City State)"),
        CountryData("vc", "+1", "Saint Vincent &amp; The Grenadines"),
        CountryData("ve", "+58", "Venezuela, Bolivarian Republic Of"),
        CountryData("vg", "+1", "British Virgin Islands"),
        CountryData("vi", "+1", "US Virgin Islands"),
        CountryData("vn", "+84", "Vietnam"),
        CountryData("vu", "+678", "Vanuatu"),
        CountryData("wf", "+681", "Wallis And Futuna"),
        CountryData("ws", "4685", "Samoa"),
        CountryData("xk", "+383", "Kosovo"),
        CountryData("ye", "+967", "Yemen"),
        CountryData("yt", "+262", "Mayotte"),
        CountryData("za", "+27", "South Africa"),
        CountryData("zm", "+260", "Zambia"),
        CountryData("zw", "+263", "Zimbabwe"),
    )
    const val AFGHANISTAN_HINT = "70 123 4567"
    const val ALGERIA_HINT = "551 23 45 67"
    const val AMERICAN_SAMOA_HINT = "684-733-1234"
    const val UNITED_ARAB_EMIRATES_HINT = "50 123 4567"
    const val ALAND_ISLANDS_HINT = "41 2345678"
    const val UNITED_STATES_AMERICA_HINT = "201-555-0123"
    const val ANDORRA_HINT = "312 345"
    const val ANGOLA_HINT = "923 123 456"
    const val ANTIGUA_AND_BARBUDA_HINT = "268-464-1234"
    const val ARGENTINA_HINT = "9 11 2345-6789"
    const val ALBANIA_HINT = "67 212 3456"
    const val ARUBA_HINT = "560 1234"
    const val AUSTRALIA_HINT = "412 345 678"
    const val AUSTRIA_HINT = "664 123456"
    const val AZERBAIJAN_HINT = "40 123 45 67"
    const val SAINT_MARTIN_HINT = "690 00 12 34"
    const val BAHAMAS_HINT = "242-359-1234"
    const val BAHRAIN_HINT = "3600 1234"
    const val BANGLADESH_HINT = "1812-345678"
    const val BARBADOS_HINT = "246-250-1234"
    const val BELARUS_HINT = "29 491-19-11"
    const val BELGIUM_HINT = "470 12 34 56"
    const val BELIZE_HINT = "622-1234"
    const val BENIN_HINT = "90 01 12 34"
    const val BERMUDA_HINT = "441-370-1234"
    const val BHUTAN_HINT = "17 12 34 56"
    const val UNITED_KINGDOM_HINT = "7400 123456"
    const val BOLIVIA_HINT = "71234567"
    const val BOSNIA_HINT = "61 123 456"
    const val BOTSWANA_HINT = "71 123 456"
    const val BRAZIL_HINT = "11 96123-4567"
    const val BRUNEI_DARUSSALAM_HINT = "712 3456"
    const val BULGARIA_HINT = "48 123 456"
    const val BURKINA_FASO_HINT = "70 12 34 56"
    const val CAPE_VERDE_HINT = "991 12 34"
    const val CAYMAN_ISLANDS_HINT = "345-323-1234"
    const val DJIBOUTI_HINT = "77 83 10 01"
    const val COCOS_HINT = "412 345 678"
    const val COOK_ISLANDS_HINT = "71 234"
    const val CURACAO_HINT = "9 518 1234"
    const val CHAD_HINT = "63 01 23 45"
    const val CZECH_REPUBLIC_HINT = "601 123 456"
    const val CHINA_HINT = "131 2345 6789"
    const val DENMARK_HINT = "32 12 34 56"
    const val DOMINICAN_REPUBLIC_HINT = "809-234-5678"
    const val DOMINICA_HINT = "767-225-1234"
    const val ECUADOR_HINT = "99 123 4567"
    const val EQUATORIAL_GUINEA_HINT = "222 123 456"
    const val EL_SALVADOR_HINT = "7012 3456"
    const val ERITREA_HINT = "7 123 456"
    const val ARMENIA_HINT = "77 123456"
    const val ESTONIA_HINT = "5123 4567"
    const val ETHIOPIA_HINT = "91 123 4567"
    const val FALKLAND_ISLANDS_HINT = "51234"
    const val FAROE_ISLANDS_HINT = "211234"
    const val MOROCCO_HINT = "650-123456"
    const val TURKEY_HINT = "501 234 56 78"
    const val PHILIPPINES_HINT = "905 123 4567"
    const val STATE_OF_PALESTINE_HINT = "599 123 456"
    const val FRANCE_HINT = "6 12 34 56 78"
    const val FRENCH_GUIANA_HINT = "694 20 12 34"
    const val FRENCH_POLYNESIA_HINT = "87 12 34 56"
    const val GABON_HINT = "6 03 12 34"
    const val GAMBIA_HINT = "301 2345"
    const val GHANA_HINT = "23 123 4567"
    const val GUINEA_HINT = "601 12 34 56"
    const val GUINEA_BISSAU_HINT = "955 012 345"
    const val GRENADA_HINT = "473-403-1234"
    const val GREENLAND_HINT = "22 12 34"
    const val GUADELOUPE_HINT = "690 00 12 34"
    const val GUAM_HINT = "671-300-1234"
    const val GUATEMALA_HINT = "5123 4567"
    const val GUERNSEY_HINT = "7781 123456"
    const val GUYANA_HINT = "609 1234"
    const val SOUTH_AFRICA_HINT = "71 123 4567"
    const val SOUTH_KOREA_HINT = "2000-0000"
    const val SOUTH_SUDAN_HINT = "977 123 456"
    const val GEORGIA_HINT = "555 12 34 56"
    const val HAITI_HINT = "34 10 1234"
    const val CROATIA_HINT = "92 123 4567"
    const val NETHERLANDS_HINT = "6 12345678"
    const val HONDURAS_HINT = "9123-4567"
    const val HONG_KONG_HINT = "5123 4567"
    const val VIRGIN_ISLANDS_HINT = "284-300-1234"
    const val SPAIN_HINT = "612 34 56 78"
    const val SWEDEN_HINT = "70 123 45 67"
    const val SWITZERLAND_HINT = "78 123 45 67"
    const val JAPAN_HINT = "-1234-5678"
    const val JERSEY_HINT = "7797 712345"
    const val CAMBODIA_HINT = "91 234 567"
    const val CAMEROON_HINT = "6 71 23 45 67"
    const val CANADA_HINT = "506-234-5678"
    const val MONTENEGRO_HINT = "67 622 901"
    const val QATAR_HINT = "3312 3456"
    const val KAZAKHSTAN_HINT = "771 000 9998"
    const val KENYA_HINT = "712 123456"
    const val CYPRUS_HINT = "96 123456"
    const val KYRGYZSTAN_HINT = "700 123 456"
    const val COLOMBIA_HINT = "321 1234567"
    const val COMOROS_HINT = "321 23 45"
    const val CONGO_HINT = "61234567"
    const val CONGO_DEMOCRATIC_HINT = "991 234 567"
    const val KOSOVO_HINT = "43 201 234"
    const val COSTA_RICA_HINT = "8312 3456"
    const val KUWAIT_HINT = "500 12345"
    const val NORTH_KOREA_HINT = "192 123 4567"
    const val NORTH_MACEDONIA_HINT = "72 345 678"
    const val NORTHERN_MARIANA_HINT = "670-234-5678"
    const val CUBA_HINT = "5 1234567"
    const val LAOS_HINT = "20 23 123 456"
    const val LESOTHO_HINT = "5012 3456"
    const val LATVIA_HINT = "21 234 567"
    const val LIBERIA_HINT = "77 012 3456"
    const val LIBYA_HINT = "91-2345678"
    const val LITHUANIA_HINT = "612 34567"
    const val LEBANON_HINT = "71 123 456"
    const val LUXEMBOURG_HINT = "628 123 456"
    const val HUNGARY_HINT = "20 123 4567"
    const val MACAU_HINT = "6612 3456"
    const val MADAGASCAR_HINT = "32 12 345 67"
    const val MALAWI_HINT = "991 23 45 67"
    const val MALDIVES_HINT = "771-2345"
    const val MALAYSIA_HINT = "12-345 6789"
    const val MALI_HINT = "65 01 23 45"
    const val MALTA_HINT = "9696 1234"
    const val MARSHALL_ISLANDS_HINT = "235-1234"
    const val MARTINIQUE_HINT = "696 20 12 34"
    const val MAURITIUS_HINT = "5251 2345"
    const val MAYOTTE_HINT = "639 01 23 45"
    const val MEXICO_HINT = "222 123 4567"
    const val EGYPT_HINT = "100 123 4567"
    const val MICRO_HINT = "350 1234"
    const val MONGOLIA_HINT = "8812 3456"
    const val MOLDOVA_HINT = "621 12 345"
    const val MONACO_HINT = "6 12 34 56 78"
    const val MONTSERRAT_HINT = "664-492-3456"
    const val MAURITANIA_HINT = "22 12 34 56"
    const val MOZAMBIQUE_HINT = "82 123 4567"
    const val MYANMAR_HINT = "9 712 345 678"
    const val NAMIBIA_HINT = "81 123 4567"
    const val NAURU_HINT = "555 1234"
    const val NEPAL_HINT = "984-1234567"
    const val NIGER_HINT = "93 12 34 56"
    const val NIGERIA_HINT = "802 123 4567"
    const val NIUE_HINT = "888 4012"
    const val CHRISTMAS_ISLAND_HINT = "412 345 678"
    const val NORFOLK_HINT = "3 81234"
    const val NORWAY_HINT = "406 12 345"
    const val CENTRAL_AFRICAN_HINT = "70 01 23 45"
    const val UZBEKISTAN_HINT = "91 234 56 78"
    const val PAKISTAN_HINT = "301 2345678"
    const val PALAU_HINT = "620 1234"
    const val PANAMA_HINT = "6123-4567"
    const val PAPUA_NEW_GUINEA_HINT = "7012 3456"
    const val PARAGUAY_HINT = "961 456789"
    const val PERU_HINT = "912 345 678"
    const val POLAND_HINT = "512 345 678"
    const val PORTUGAL_HINT = "912 345 678"
    const val PUERTO_RICO_HINT = "787-234-5678"
    const val REUNION_HINT = "692 12 34 56"
    const val ROMANIA_HINT = "712 034 567"
    const val RWANDA_HINT = "720 123 456"
    const val RUSSIA_HINT = "912 345-67-89"
    const val SAINT_BARHELEMY_HINT = "690 00 12 34"
    const val SAINT_HELENA_HINT = "51234"
    const val SAINT_KITTS_HINT = "869-765-2917"
    const val SAINT_LUCIA_HINT = "758-284-5678"
    const val SAINT_PIERRE_HINT = "55 12 34"
    const val SAINT_VINCENT_HINT = "784-430-1234"
    const val SAMOA_HINT = "72 12345"
    const val SAN_MARINO_HINT = "66 66 12 12"
    const val SAO_TOME_HINT = "981 2345"
    const val SENEGAL_HINT = "70 123 45 67"
    const val SEYCHELLES_HINT = "2 510 123"
    const val SERBIA_HINT = "60 1234567"
    const val SIERRA_LEONE_HINT = "25 123456"
    const val SINGAPORE_HINT = "8123 4567"
    const val SINT_MAARTEN_HINT = "721-520-5678"
    const val SLOVAKIA_HINT = "912 123 456"
    const val SOLOMON_ISLANDS_HINT = "74 21234"
    const val SOMALI_HINT = "7 1123456"
    const val SRI_LANKA_HINT = "71 234 5678"
    const val SUDAN_HINT = "91 123 1234"
    const val SURINAME_HINT = "741-2345"
    const val SYRIAN_HINT = "944 567 890"
    const val SAUDI_ARABIA_HINT = "51 234 5678"
    const val SWAZILAND_HINT = "7612 3456"
    const val CHILE_HINT = "2 2123 4567"
    const val TAIJIKISTAN_HINT = "917 12 3456"
    const val TAZMANIA_HINT = "621 234 567"
    const val THAILAND_HINT = "81 234 5678"
    const val TAIWAN_HINT = "912 345 678"
    const val TIMOR_LESTE_HINT = "7721 2345"
    const val TOGO_HINT = "90 11 23 45"
    const val TOKELAU_HINT = "7290"
    const val TONGA_HINT = "771 5123"
    const val TRINIDAD_AND_TOBAGO_HINT = "868-291-1234"
    const val TUNISIA_HINT = "20 123 456"
    const val TURKS_AND_CAICOS_HINT = "649-231-1234"
    const val TUVALU_HINT = "901234"
    const val TURKMENISTAN_HINT = "66 123456"
    const val UGANDA_HINT = "712 345678"
    const val UKRAINA_HINT = "50 123 4567"
    const val OMAN_HINT = "9212 3456"
    const val URUGUAY_HINT = "94 231 234"
    const val JORDAN_HINT = "7 9012 3456"
    const val VANUATU_HINT = "591 2345"
    const val VENEZUELA_HINT = "412-1234567"
    const val VIETNAM_HINT = "91 234 56 78"
    const val WALLI_AND_FATUNA_HINT = "82 12 34"
    const val YEMEN_HINT = "712 345 678"
    const val NEW_CALEDONIA_HINT = "75.12.34"
    const val NEW_ZEALAND_HINT = "21 123 4567"
    const val GREECE_HINT = "691 234 5678"
    const val ZAMBIA_HINT = "95 5123456"
    const val ZIMBABWE_HINT = "71 234 5678"
    const val ANGUILLA_HINT = "(264) 123-1234"
    const val ANTARCTICA_HINT = "xxx xxx"
    const val BURUNDI_HINT = "29 12 34 56"
    const val COTE_DLVOIRE_HINT = "02 12 34 56"
    const val GERMANY_HINT = "0171 123456"
    const val FINLAND_HINT = "042 123456"
    const val FIJI_HINT = "665 1234"
    const val ISRAEL_HINT = "03-123-4567"
    const val IRELAND_HINT = "087 123 4567"
    const val INDONESIA_HINT = "21 8379 6236"
    const val INDIA_HINT = "241 123 5"
    const val IRAN_HINT = "021 1234 5678"
    const val ITALY_HINT = "388 123 4567"
    const val JAMAICA_HINT = "(876) 123-4567"
    const val IRAQ_HINT = "0777 123 4567"
    const val SLOVENIA_HINT = "070 123 456"
    val DEFAULT_LANG_VODE = "com"
    val DEFAULT_PHONE = "+7"

}
